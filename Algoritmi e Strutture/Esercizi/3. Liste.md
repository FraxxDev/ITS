...
```python
# Inserimento di N valori nella lista vuota con stampa di elementi
...
INIZIO
	...
	for i da 0 a n-1
		leggi numero_input
		AGGIUNGI(L, numero_input)
		
	for i da 0 a lunghezza(L)-1
		SCRIVI L[i]
FINE
```
...
```python
# Data una lista di N numeri interi, sommare i soli valori positivi presenti e calcolare la media di essi.
...
INIZIO
	...
	per i da 0 a lunghezza(L)-1
		SE L[i] >= 0
			contPositivi <- contPositivi + 1
			somma <- somma + L[i]
			
	SE contPositivi > 0
		media <- somma / contPositivi
	
		SCRIVI "la somma dei numeri positivi è ", somma
		SCRIVI "la media dei numeri positivi è ", media
	ALTRIMENTI
		SCRIVI "non sono stati inseriti numeri positivi."
FINE
```

```python
# Data una lista non vuota di N elementi, cercare un valore X (la prima corrispondenza) e sampare la posizione all'interno della lista stessa
...
per i da 0 a lunghezza(L)-1
	SE x == L[i]
		SCRIVI "il numero si trova in posizione ", i
		trovato <- true
		esci dal ciclo
